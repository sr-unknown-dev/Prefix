...
class PrefixManager
{
        ...
        if (isset($data["players"])) {
            foreach ($data["players"] as $playerName => $playerData) {
                if (isset($playerData["prefix"])) {
                    $this->prefixes[$playerName] = new Tag($playerData["prefix"]);
                }
            }
        }
    }

    public function setTag(Player $player, Tag $tag): void
    {
        $this->prefixes[$player->getName()] = $tag;
        $this->save();
    }
    ...
    {
        $data = ["players" => []];
        foreach ($this->prefixes as $playerName => $tag) {
