...
use Closure;
use Logger;
use muqsit\tebex\event\TebexExecuteOfflineCommandEvent;
...
final class TebexDueOfflineCommandsHandler{
	...
	private function onFetchDueOfflineCommands(TebexQueuedOfflineCommandsInfo $info) : void{
		...
		foreach($commands as $command){
			$this->executeCommand($command, function(?string $command_string) use($command) : void{
	...
	private function instantlyExecuteCommand(TebexQueuedOfflineCommand $command) : ?string{
		$original_placeholders = TebexApiUtils::offlineCommandParameters($command->player);
		$event = new TebexExecuteOfflineCommandEvent($this->plugin, $command->player, $command, $original_placeholders, $original_placeholders);
		$event->call();
		if($event->isCancelled()){
			return null;
		}
		$command_string = $event->getFinalCommand();
		if(!Server::getInstance()->dispatchCommand(TebexCommandSender::getInstance(), $command_string)){
			return null;
		}
